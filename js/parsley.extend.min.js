...
return a>=b[0]&&a<=b[1]},greaterthan:function(a,b,c){c.options.validateIfUnchanged=!0;return new Number(a)>new Number(d(b).val())},lessthan:function(a,b,c){c.options.validateIfUnchanged=!0;return new Number(a)<new Number(d(b).val())},beforedate:function(a,b){return Date.parse(a)<Date.parse(d(b).val())},afterdate:function(a,b){split = d(b).val().split('/'); nd = [split[1], split[0], split[2]].join('/');split = a.split('/'); na = [split[1], split[0], split[2]].join('/');return Date.parse(nd)<=Date.parse(na)},inlist:function(a,b,c){return-1!==(b+"").split(RegExp("\\s*\\"+(c.options.inlistDelimiter||",")+"\\s*")).indexOf(a.trim())},luhn:function(a){a=
